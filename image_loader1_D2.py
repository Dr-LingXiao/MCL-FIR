...
class TripletGenerator_D2(object):
    def __init__(self, root, base_path, meta, CLIP_text):
...
class TripletImageLoader_D2(torch.utils.data.Dataset):

    def __init__(self, root, base_path, num_triplets, CLIP_text, transform=None,
                 loader=default_image_loader):
        self.root = root
        self.base_path = base_path
        self.num_triplets = num_triplets

        self.meta = MetaLoader_D2(self.root, self.base_path)
        self.CLIP_text = CLIP_text
        self.triplet_generator = TripletGenerator_D2(self.root, self.base_path, self.meta.data, self.CLIP_text)
    ...
    def __getitem__(self, index):
        ...
        return img1, img2, img3, c, c_text
...
class ImageLoader_D2(torch.utils.data.Dataset):
    
    def __init__(self, root, base_path, filenames_filename, split, cand_query, CLIP_text,CLIP_text0, CLIP_text1, transform=None, loader=default_image_loader):
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(os.path.join(self.root, self.base_path, self.filenamelist[int(path)])):
            ...
            return img, c, self.CLIP_text0[c], self.CLIP_text1[c], gdtruth, self.filenamelist[int(path)].strip().split('/')[-1].split('.')[0]   #imgdata, condition, groundtruth, imgid(for retrieval use)
        else:
            return None

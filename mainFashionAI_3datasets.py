...
from image_loader import TripletImageLoader as TripletImageLoader
from image_loader import ImageLoader as ImageLoader
from image_loader import MetaLoader as MetaLoader

from image_loader_D import TripletImageLoader as TripletImageLoader_D
from image_loader_D import ImageLoader as ImageLoader_D
from image_loader_D import MetaLoader as MetaLoader_D

from image_loader_D2 import TripletImageLoader as TripletImageLoader_D2
from image_loader_D2 import ImageLoader as ImageLoader_D2
from image_loader_D2 import MetaLoader as MetaLoader_D2
...
parser.add_argument('--num_triplets', type=int, default=20000, metavar='N',
...
def train(train_loader, tnet, criterion, optimizer, task_id, epoch, awl):
    ...
    tnet.train()
    for batch_idx, (data1, data2, data3, c, c_text, c_text0, c_text1, data11, data22, data33) in enumerate(train_loader):
        if args.cuda:
            data1, data2, data3, c, c_text, c_text0, c_text1, data11, data22, data33 = data1.cuda(), data2.cuda(), data3.cuda(), c.cuda(), c_text.cuda(), c_text0.cuda(), c_text1.cuda(), data11.cuda(), data22.cuda(), data33.cuda()
        # compute similarity
        sim_a, sim_b, loss_c, loss_kd = tnet(data1, data2, data3, c_text, c_text0, c_text1, data11, data22, data33)

        # loss = loss_triplet
        # loss_sum = 0.9 * loss + 0.1 * loss_noise
        # measure accuracy and record loss
        acc = accuracy(sim_a, sim_b)
        loss_sum = loss_c + 0.0001*loss_kd
        losses.update(loss_sum.data.item(), data1.size(0))
...
def test(test_candidate_loader, test_query_loader, test_model, task_id, attributes, dataset, epoch=-1):
    global meta
    ...
    for _, (img, c, c_text0, c_text1, gdtruth, _) in enumerate(test_candidate_loader):
        ...
        masked_embedding = test_model(img, c_text0, c_text1, True)
        ...
        elif dataset == args.second_dataset:
            for i in range(masked_embedding.size(0)):
    ...
    # for attribute in attributes:
    if dataset == args.dataset:
        mAP = mean_average_precision(cand_set[task_id], queries[task_id], c_gdtruth[task_id], q_gdtruth[task_id])
        mAPs.update(mAP, queries[task_id].shape[0])
        
    elif dataset == args.second_dataset:
        second_task_id = task_id-8
        mAP = mean_average_precision(cand_set_D[second_task_id], queries_D[second_task_id], c_gdtruth_D[second_task_id], q_gdtruth_D[second_task_id])
        mAPs.update(mAP, queries_D[second_task_id].shape[0])
    else:
        third_task_id = task_id-13
        mAP = mean_average_precision(cand_set_D2[third_task_id], queries_D2[third_task_id], c_gdtruth_D2[third_task_id],
                                     q_gdtruth_D2[third_task_id])
        mAPs.update(mAP, queries_D2[third_task_id].shape[0])
        
    mAP_cs[task_id].update(mAP)
...
def save_checkpoint(state, is_best, task_id, filename='checkpoint.pth.tar'):
    ...
    directory = "runs/%s/"%(task_id)
    if not os.path.exists(directory):
        os.makedirs(directory)
    filename = directory + filename
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'runs/%s/'%(task_id) + 'model_best.pth.tar')
        
        

def save_backbonenet(state, is_best, task_id, filename='checkpoint.pth.tar'):
    """Saves checkpoint to disk"""
    directory = "runs_backbone/"
    if not os.path.exists(directory):
        os.makedirs(directory)
    filename = directory + filename
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'runs_backbone/' + 'model_best.pth.tar')
        

def save_head(state, is_best, task_id, filename='checkpoint.pth.tar'):
    """Saves checkpoint to disk"""
    directory = "runs_head/%s/"%(task_id)
    if not os.path.exists(directory):
        os.makedirs(directory)
    filename = directory + filename
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'runs_head/%s/'%(task_id) + 'model_best.pth.tar')
...
def main():
    ...
    print(len(meta.data['ATTRIBUTES']))
    print(len(meta1.data['ATTRIBUTES']))
    print(len(meta2.data['ATTRIBUTES']))
    ...
    for t in range(len(attributes)):
        ...
        Sub_networks.append(copy.deepcopy(enet))
    ...
    if args.test:
        ...
        test_mAP = final_test(test_candidate_loader, test_query_loader, Sub_networks)
        sys.exit()
    ...
    for task_id in range(args.num_tasks):
        best_mAP = 0
        tnet = get_model('Tripletnet')(Sub_networks[task_id], args.dim_embed, len(attributes))
        tnet.cuda()
        ...
        if task_id < 8:
            ...
            train_loader = torch.utils.data.DataLoader(
                TripletImageLoader(args.data_path, args.dataset, args.num_triplets, task_id, CLIP_text[:8], CLIP_text0[:8],
                                   CLIP_text1[:8],
                                   transform=transforms.Compose([
                                       transforms.Resize(224),
                                       transforms.CenterCrop(224),
            ...
            val_candidate_loader = torch.utils.data.DataLoader(
                ImageLoader(args.data_path, args.dataset, 'filenames_valid.txt',
                            'valid', 'candidate', task_id, CLIP_text[:8], CLIP_text0[:8], CLIP_text1[:8],
                            transform=transforms.Compose([
                                transforms.Resize(224),
                                transforms.CenterCrop(224),
                                transforms.ToTensor(),
            ...
            val_query_loader = torch.utils.data.DataLoader(
                ImageLoader(args.data_path, args.dataset, 'filenames_valid.txt',
                            'valid', 'query', task_id, CLIP_text[:8], CLIP_text0[:8], CLIP_text1[:8],
                            transform=transforms.Compose([
                                transforms.Resize(224),
                                transforms.CenterCrop(224),
                                transforms.ToTensor(),
                ...
                batch_size=args.batch_size, shuffle=True, **kwargs)
        elif 7 < task_id and task_id < 13:
            dataset = args.second_dataset
            second_task_id = task_id-8
            train_loader = torch.utils.data.DataLoader(
                TripletImageLoader_D(args.data_path, args.second_dataset, args.num_triplets, second_task_id, CLIP_text[8:13],
                                     CLIP_text0[8:13], CLIP_text1[8:13],
                                     transform=transforms.Compose([
                                         transforms.Resize(224),
                                         transforms.CenterCrop(224),
                                         transforms.RandomHorizontalFlip(),
            ...
            val_candidate_loader = torch.utils.data.DataLoader(
                ImageLoader_D(args.data_path, args.second_dataset, 'filenames_valid.txt',
                              'valid', 'candidate', second_task_id, CLIP_text[8:13], CLIP_text0[8:13], CLIP_text1[8:13],
                              transform=transforms.Compose([
                                  transforms.Resize(224),
                                  transforms.CenterCrop(224),
                                  transforms.ToTensor(),
            ...
            val_query_loader = torch.utils.data.DataLoader(
                ImageLoader_D(args.data_path, args.second_dataset, 'filenames_valid.txt',
                              'valid', 'query', second_task_id, CLIP_text[8:13], CLIP_text0[8:13], CLIP_text1[8:13],
                              transform=transforms.Compose([
                                  transforms.Resize(224),
                                  transforms.CenterCrop(224),
                                  transforms.ToTensor(),
            ...
            dataset = args.third_dataset
            third_task_id = task_id-13
            train_loader = torch.utils.data.DataLoader(
                TripletImageLoader_D2(args.data_path, args.third_dataset, args.num_triplets, third_task_id, CLIP_text[13:],
                                      CLIP_text0[13:], CLIP_text1[13:],
                                      transform=transforms.Compose([
                                          transforms.Resize(224),
                                          transforms.CenterCrop(224),
                                          transforms.RandomHorizontalFlip(),
            ...
            val_candidate_loader = torch.utils.data.DataLoader(
                ImageLoader_D2(args.data_path, args.third_dataset, 'filenames_valid.txt',
                               'valid', 'candidate', third_task_id, CLIP_text[13:], CLIP_text0[13:], CLIP_text1[13:],
                               transform=transforms.Compose([
                                   transforms.Resize(224),
                                   transforms.CenterCrop(224),
                                   transforms.ToTensor(),
            ...
            val_query_loader = torch.utils.data.DataLoader(
                ImageLoader_D2(args.data_path, args.third_dataset, 'filenames_valid.txt',
                               'valid', 'query', third_task_id, CLIP_text[13:], CLIP_text0[13:], CLIP_text1[13:],
                               transform=transforms.Compose([
                                   transforms.Resize(224),
                                   transforms.CenterCrop(224),
                                   transforms.ToTensor(),
        ...
        for epoch in range(args.start_epoch, args.epochs + 1):
            # train for one epoch
            train(train_loader, tnet, criterion, optimizer, task_id, epoch, awl)
            train_loader.dataset.refresh()
            # evaluate on validation set
            mAP = test(val_candidate_loader, val_query_loader, Sub_networks[task_id], task_id, attributes, dataset, epoch)
            print(mAP)
            ...
            is_best_all = mAP > best_mAP_all
            best_mAP_all = max(mAP, best_mAP_all)
            # print('map:', mAP)
            ...
            save_backbonenet({
                'epoch': epoch,
                'state_dict': Sub_networks[task_id].backbonenet.state_dict(),
                'prec': mAP,
            }, is_best_all, task_id)
               
            # Save only the head network
            state_dict = deepcopy(Sub_networks[task_id].state_dict())
            for key in list(state_dict.keys()):
                if key.startswith("backbonenet."):  # Only remove backbone keys
                    del state_dict[key]

...
from image_loader_D import TripletImageLoader_D, ImageLoader_D, MetaLoader_D
import copy
...
parser.add_argument('--num_tasks', default=13, type=int, help='number of sequential tasks for FashionAI')
parser.add_argument('--num_attributes', default=13, type=int, help='number of sequential tasks for FashionAI')
...
def train(train_loader, tnet, criterion, optimizer, task_id, epoch, awl):
    ...
    tnet.train()
    for batch_idx, (data1, data2, data3, c, c_text, c_text0, c_text1, data1_1, data2_1, data3_1) in enumerate(train_loader):
        if args.cuda:
            data1, data2, data3, c, c_text, c_text0, c_text1 = data1.cuda(), data2.cuda(), data3.cuda(), c.cuda(), c_text.cuda(), c_text0.cuda(), c_text1.cuda()
            data1_1, data2_1, data3_1 = data1_1.cuda(), data2_1.cuda(), data3_1.cuda()
        # compute similarity
        sim_a, sim_b, sim_a1, sim_b1, loss_c = tnet(data1, data2, data3, c_text, c_text0, c_text1, data1_1, data2_1, data3_1)
        ...
        loss_triplet = (criterion(sim_a, sim_b, target)+criterion(sim_a1, sim_b1, target))/2.
        loss = loss_triplet
...
def test(test_candidate_loader, test_query_loader, test_model, task_id, attributes, dataset, epoch=-1):
    ...
    cand_set_D = [[] for _ in attributes[8:]]
    c_gdtruth_D = [[] for _ in attributes[8:]]
    for _, (img, c, c_text0, c_text1, gdtruth, _) in enumerate(test_candidate_loader):
        ...
        masked_embedding = test_model(img, c_text0, c_text1)

        if dataset == "FashionAI":
    ...
    queries_D = [[] for _ in attributes[8:]]
    q_gdtruth_D = [[] for _ in attributes[8:]]
    ...
    if dataset == "FashionAI":
        ...
        mAP = mean_average_precision(cand_set_D[task_id-8], queries_D[task_id-8], c_gdtruth_D[task_id-8], q_gdtruth_D[task_id-8])
        mAPs.update(mAP, queries_D[task_id-8].shape[0])
...
def final_test(test_candidate_loader, test_query_loader,test_candidate_loader_D, test_query_loader_D, Sub_networks):
    global meta
    ...
    for _, (img, c, gdtruth, _) in enumerate(test_candidate_loader):
        ...
        for i in range(c.size(0)):
            masked_embedding.append(Sub_networks[c[i:i+1].item()](img[i,:].unsqueeze(dim=0), c[i:i+1]))
    ...
    for _, (img, c, gdtruth, _) in enumerate(test_candidate_loader_D):

        if args.cuda:
            img, c = img.cuda(), c.cuda()
        masked_embedding = []
        for i in range(c.size(0)):
            masked_embedding.append(Sub_networks[c[i:i+1].item() +8](img[i,:].unsqueeze(dim=0), c[i:i+1]))
        masked_embedding = torch.cat(masked_embedding, dim=0)
    ...
    for _, (img, c, gdtruth, _) in enumerate(test_query_loader_D):
        if args.cuda:
            img, c = img.cuda(), c.cuda()
    ...
    for attribute in attributes:
        if dataset == "FashionAI":
            ...
            mAPs.update(mAP, queries_D[attribute - 8].shape[0])
...
def save_checkpoint_test(state, is_best, task_id, filename='checkpoint.pth.tar'):
    """Saves checkpoint to disk"""
    directory = "runs/test/%s/"%(task_id)
    if not os.path.exists(directory):
        os.makedirs(directory)
    filename = directory + filename
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'runs/test/%s/'%(task_id) + 'model_best.pth.tar')
...
def main():
    ...
    global attributes
    attributes = [i for i in range(len(meta.data['ATTRIBUTES']) + len(meta1.data['ATTRIBUTES']))]
        ...
        for epoch in range(args.start_epoch, args.epochs + 1):
            if task_id < 8:
                ...
                train_loader = torch.utils.data.DataLoader(
                    TripletImageLoader(args.data_path, args.dataset, args.num_triplets, task_id, CLIP_text, CLIP_text0, CLIP_text1,
                                       transform=transforms.Compose([
                                           transforms.Resize(224),
                                           transforms.CenterCrop(224),
                ...
                val_candidate_loader = torch.utils.data.DataLoader(
                    ImageLoader(args.data_path, args.dataset, 'filenames_valid.txt',
                                'valid', 'candidate',task_id, CLIP_text, CLIP_text0, CLIP_text1,
                                transform=transforms.Compose([
                                    transforms.Resize(224),
                                    transforms.CenterCrop(224),
                                    transforms.ToTensor(),
                ...
                val_query_loader = torch.utils.data.DataLoader(
                    ImageLoader(args.data_path, args.dataset, 'filenames_valid.txt',
                                'valid', 'query',task_id, CLIP_text, CLIP_text0, CLIP_text1,
                                transform=transforms.Compose([
                                    transforms.Resize(224),
                                    transforms.CenterCrop(224),
                                    transforms.ToTensor(),
                ...
                train_loader = torch.utils.data.DataLoader(
                    TripletImageLoader_D(args.data_path, args.second_dataset, args.num_triplets, task_id, CLIP_text, CLIP_text0, CLIP_text1,
                                       transform=transforms.Compose([
                                           transforms.Resize(224),
                                           transforms.CenterCrop(224),
                                           transforms.RandomHorizontalFlip(),
            ...
            save_checkpoint_test({
                'epoch': epoch,
                'state_dict': Sub_networks[task_id].state_dict(),
                'prec': mAP,
            }, is_best, task_id)

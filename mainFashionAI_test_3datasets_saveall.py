...
def final_test(test_candidate_loader, test_query_loader,test_candidate_loader_D, test_query_loader_D,test_candidate_loader_D2, test_query_loader_D2, Sub_networks):
    ...
    for i in range(len(attributes)):
        Sub_networks[i].embeddingnet.cuda().eval()
    ...
    for _, (img, c, c_text0, c_text1, gdtruth, _) in enumerate(test_candidate_loader):
        ...
        for i in range(c.size(0)):
            masked_embedding.append(Sub_networks[c[i:i+1].item()].embeddingnet(img[i,:].unsqueeze(dim=0), c_text0[i:i+1], c_text1[i:i+1], True))
    ...
    for _, (img, c, c_text0, c_text1, gdtruth, _) in enumerate(test_candidate_loader_D):
        ...
        masked_embedding = []
        for i in range(c.size(0)):
            masked_embedding.append(Sub_networks[c[i:i+1].item() +8].embeddingnet(img[i,:].unsqueeze(dim=0), c_text0[i:i+1], c_text1[i:i+1], True))
        masked_embedding = torch.cat(masked_embedding, dim=0)
    ...
    for _, (img, c, c_text0, c_text1, gdtruth, _) in enumerate(test_candidate_loader_D2):
        ...
        masked_embedding = []
        for i in range(c.size(0)):
            masked_embedding.append(Sub_networks[c[i:i+1].item() +13].embeddingnet(img[i,:].unsqueeze(dim=0), c_text0[i:i+1], c_text1[i:i+1], True))
        masked_embedding = torch.cat(masked_embedding, dim=0)
    ...
    for _, (img, c, c_text0, c_text1, gdtruth, _) in enumerate(test_query_loader_D2):
        ...
        for i in range(c.size(0)):
            masked_embedding.append(
                Sub_networks[c[i:i + 1].item() + 13].embeddingnet(img[i, :].unsqueeze(dim=0), c_text0[i:i + 1], c_text1[i:i + 1], True))
        masked_embedding = torch.cat(masked_embedding, dim=0)
...
def main():
    ...
    if args.test:
        for t in range(len(attributes)):
            enet = get_model(args.model)(backbone, n_attributes=len(attributes), embedding_size=args.dim_embed, decay=0.999)
            tnet = get_model('Tripletnet')(enet, args.dim_embed, len(attributes))
            # Sub_networks.append(copy.deepcopy(enet))
            t_nets.append(copy.deepcopy(tnet))
            checkpoint = torch.load('runs/%s/' % (t) + 'model_best.pth.tar')
            t_nets[t].load_state_dict(checkpoint['state_dict'], strict=False)
        ...
        test_mAP = final_test(test_candidate_loader, test_query_loader,test_candidate_loader_D, test_query_loader_D, test_candidate_loader_D2, test_query_loader_D2, t_nets)

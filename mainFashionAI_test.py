...
from image_loader1 import TripletImageLoader, ImageLoader, MetaLoader
from image_loader1_D import TripletImageLoader_D, ImageLoader_D, MetaLoader_D
import copy
...
def test(test_candidate_loader, test_query_loader, test_model, task_id, attributes, epoch=-1):
    global meta
...
def final_test(test_candidate_loader, test_query_loader,test_candidate_loader_D, test_query_loader_D, Sub_networks):
    ...
    for _, (img, c, c_text0, c_text1, gdtruth, _) in enumerate(test_candidate_loader):
        ...
        for i in range(c.size(0)):
            masked_embedding.append(Sub_networks[c[i:i+1].item()](img[i,:].unsqueeze(dim=0), c_text0[i:i+1], c_text1[i:i+1]))
    ...
    for _, (img, c, c_text0, c_text1, gdtruth, _) in enumerate(test_candidate_loader_D):
        ...
        masked_embedding = []
        for i in range(c.size(0)):
            masked_embedding.append(Sub_networks[c[i:i+1].item() +8](img[i,:].unsqueeze(dim=0), c_text0[i:i+1], c_text1[i:i+1]))
        masked_embedding = torch.cat(masked_embedding, dim=0)
...
def main():
    ...
    if args.test:
        ...
        test_query_loader = torch.utils.data.DataLoader(
            ImageLoader(args.data_path, args.dataset, 'filenames_test.txt',
                'test', 'query', CLIP_text, CLIP_text0, CLIP_text1,
                            transform=transforms.Compose([
                                transforms.Resize(224),
                                transforms.CenterCrop(224),
                                transforms.ToTensor(),
        ...
        test_query_loader_D = torch.utils.data.DataLoader(
            ...
            batch_size=args.batch_size, shuffle=True, **kwargs)
        test_mAP = final_test(test_candidate_loader, test_query_loader,test_candidate_loader_D, test_query_loader_D, Sub_networks)
    ...
    for task_id in range(args.num_tasks):
        tnet = get_model('Tripletnet')(Sub_networks[task_id])
        tnet.cuda()
        parameters = filter(lambda p: p.requires_grad, tnet.parameters())
        optimizer = optim.Adam(parameters, lr=args.lr)
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=args.step_size, gamma=args.decay_rate)
        for epoch in range(args.start_epoch, args.epochs + 1):
            train_loader = torch.utils.data.DataLoader(
                TripletImageLoader(args.data_path, args.dataset, args.num_triplets, task_id,
                                   transform=transforms.Compose([
                                       transforms.Resize(224),
                                       transforms.CenterCrop(224),
            ...
            val_query_loader = torch.utils.data.DataLoader(
                ...
                batch_size=args.batch_size, shuffle=True, **kwargs)
            # train for one epoch
            train(train_loader, tnet, criterion, optimizer, task_id, epoch)
            train_loader.dataset.refresh()
            # evaluate on validation set
            mAP = test(val_candidate_loader, val_query_loader, Sub_networks[task_id], task_id, attributes, epoch)
